...
<img src="images/top-header.jpg" />
	...
	<h3>LADIES CATEGORIES</h3><br>
		<ul >
        	<?php
	mysql_connect("$host", "$user", "$pass") or die (mysql_error());	
	mysql_select_db("$db") or die(mysql_error());
	$strSQL = "SELECT DISTINCT manufacturer FROM item WHERE category= 'ladies' ";
	$rs = mysql_query($strSQL);
	while($row = mysql_fetch_array($rs))
	 {	
	 $manufacturer=$row['manufacturer'];
	 
	echo "<li><a href=mobiles.php?itemnumber=0&rows=0&category=ladies&manufacturer=".$manufacturer. ">".$manufacturer."</a></li>";

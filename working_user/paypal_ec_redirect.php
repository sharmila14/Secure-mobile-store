<?php
  ...
  $checkout=$cartname;
  $checkout .= "1"; 
  $val = "select 1 from $checkout";
	$prefloc1 = $pdoConn->prepare($val);
    $prefloc1->execute();
  
  if($prefloc1->fetch(PDO::FETCH_ASSOC) == true)
{
   
   $query13="INSERT $checkout SELECT * FROM $cartname";	
  $prefloc13 = $pdoConn->prepare($query13);
  $prefloc13->execute();
  
   $query12="TRUNCATE TABLE $cartname";	
	$prefloc12 = $pdoConn->prepare($query12);
    $prefloc12->execute();
  
   
}else{
  $query13="CREATE TABLE $checkout AS SELECT * FROM $cartname";	
  $prefloc13 = $pdoConn->prepare($query13);
  $prefloc13->execute();
	
   $query14="TRUNCATE TABLE $cartname";	
	$prefloc14 = $pdoConn->prepare($query14);
    $prefloc14->execute();
	}
   }
